...
def query_llm(model_name, prompt, max_retries=3, retry_delay=2, operation_name="LLM Query"):
...
def process_query(model_name, user_query):
    ...
    final_topics = [topic for topic in refined_topics if relevance_scores.get(topic, 0) >= 7]
    if len(final_topics) < 3:
        # Ensure we have at least 3 topics
        remaining = [t for t in refined_topics if t not in final_topics]
        remaining.sort(key=lambda t: relevance_scores.get(t, 0), reverse=True)
        final_topics.extend(remaining[:3-len(final_topics)])
    
    # Cap at maximum 7 topics to avoid excessive processing
    final_topics = final_topics[:7]
